    ...
    endpoints: (builder)=>({
        getAllAccount: builder.query<ApiResponse<Account[]>, void>({
            ...
            providesTags: (result)=> {
                if(result?.data){
                    return [
                        ...result.data.map(({id})=> ({type: 'accountApi' as const, id})),
        ...
        updateAccount: builder.mutation<ApiResponse<Account>, Account>({
            query: (account)=> ({
                url: `/${account.id}`,
                method: 'PUT',
                body: account
...
export const { useGetAllAccountQuery, useCreateAccountMutation, useUpdateAccountMutation, useDeleteAccountMutation } = accountApi